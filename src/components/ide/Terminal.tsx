...
function initState(type: TermType): TabState {
  const welcomes: Record<TermType, string> = {
    shell: "SK-Shell v1.0 — type 'help' for commands, 'run <file>' to execute files",
    python: "Python 3.12 via Pyodide — type code and press Enter • NumPy, Pandas, Math available",
    nodejs: "Node.js 18 via Piston — type JS/Node.js code and press Enter",
    java: "Java 15 via Piston — type Java statements or full class code",
    ai: "SK-AI Terminal — ask questions, get code help, explain errors. Set an API key or use Free Puter AI in Settings.",
  }
  return {
...
const ADD_OPTIONS: { type: TermType; label: string; desc: string }[] = [
  { type: "shell", label: "SK-Shell", desc: "Virtual shell · file navigation · run files" },
  { type: "python", label: "Python 3", desc: "Pyodide · offline · NumPy · Pandas" },
  { type: "nodejs", label: "Node.js", desc: "Node 18 via Piston · full JS runtime" },
  { type: "java", label: "Java", desc: "Java 15 via Piston · compile & run" },
  { type: "ai", label: "SK-AI", desc: "Ask code questions · Puter AI or API key" },
]
...
export default function MultiTerminal() {
  ...
  async function handleNodeJs(tabId: string, code: string) {
    ...
    if (runMatch) {
      ...
      addLine(tabId, "info", `Running ${filename}...`)
      const res = await runWithPiston(node.content || "", "javascript")
      if (res.output) addLines(tabId, "output", res.output.trimEnd())
      if (res.stderr) addLines(tabId, "error", res.stderr.trimEnd())
      if (!res.output && !res.stderr) addLine(tabId, "info", "(no output)")
      return
    }
    const res = await runWithPiston(trimmed, "javascript")
    if (res.output) addLines(tabId, "output", res.output.trimEnd())
    if (res.stderr) addLines(tabId, "error", res.stderr.trimEnd())
    if (!res.output && !res.stderr) addLine(tabId, "info", "(no output)")
  }
